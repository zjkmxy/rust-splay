...
impl TreeNode {
    ...
    fn new(key: i32, dir: usize, ichd: TreeNode, ochd: TreeNode) -> Self {
        TreeNode::Node {
            ...
            chd: if dir == LCHD {
                [Box::new(ichd), Box::new(ochd)]
            } else {
                [Box::new(ochd), Box::new(ichd)]
            }
        }
    }
...
impl Splay {
    fn zig(&self, dir: usize) -> Self {
        Splay {
            root: Box::new(self.root.child(dir)),
            temp: Box::new(self.temp.replace_chd(dir,
    ...
    fn finish(&self, dir: usize) -> Self {
        ...
        Splay {
            root: Box::new(self.root.replace_chd(1 ^ dir, child)),
            temp: Box::new(self.temp.replace_chd(dir, TreeNode::Null))
        }
    }
    ...
    fn remove(&self, x: i32) -> Self {
        ...
        if let TreeNode::Null = origin.root.child(RCHD) {
            return Splay {
                temp: origin.temp,
                root: Box::new(origin.root.child(LCHD))
            }
        } else {
            let sub = Splay {
                temp: origin.temp,
                root: Box::new(origin.root.child(RCHD))
            }.select(0);
            return Splay {
                temp: sub.temp,
                root: Box::new(sub.root.replace_chd(LCHD,
    ...
    fn new() -> Self {
        Splay {
            root: Box::new(TreeNode::Null),
            temp: Box::new(TreeNode::new(0, 0, TreeNode::Null, TreeNode::Null))
        }
    }
}
